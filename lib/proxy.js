...
function proxyRequest(req, res, rule) {
  ...
  if (useGateway) {
    ...
    req.url = url.parse(util.format('%s//%s:%s%s', rule.target.protocol, rule.target.host, rule.target.port, req.url)).href;
    req.headers['X-Forwarded-Url'] = req.url;

    // the proxy target is really the HTTP LAN proxy
    target = config.gateway;
    logger.info('proxy: %s %s --> %s:%s --> %s//%s:%s', req.method, req.url, config.gateway.host, config.gateway.port, rule.target.protocol, rule.target.host, rule.target.port);
  } else {
    target = rule.target;
    console.log(rule.target.port)
    logger.info('proxy: %s %s --> %s//%s:%s', req.method, req.url, rule.target.protocol, rule.target.host, rule.target.port);
  }

  var errorCallback = function errorCallback(err, proxyRequest, proxyResponse) {
    var status = 500;

    if (proxyResponse !== undefined && proxyResponse !== null) {
      status = proxyResponse.statusCode || status;
    }
